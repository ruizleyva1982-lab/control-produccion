...
.upload-box {
    background: #f0f9ff; border: 2px dashed #7dd3fc; border-radius: 12px;
    padding: 1.5rem; margin-bottom: 1rem;
}
.upload-title { color: #0369a1; font-weight: 600; font-size: 0.95rem; margin-bottom: 0.3rem; }
.upload-sub   { color: #0891b2; font-size: 0.82rem; }
.estado-ok    { background:#dcfce7; color:#166534; padding:4px 12px; border-radius:20px;
                font-size:0.8rem; font-weight:600; }
.estado-no    { background:#fee2e2; color:#991b1b; padding:4px 12px; border-radius:20px;
                font-size:0.8rem; font-weight:600; }
...
def leer_programacion_archivo(file_bytes, nombre_archivo: str) -> pd.DataFrame:
    df = pd.read_excel(io.BytesIO(file_bytes), dtype={"Cod Item": str, "Nro Documento": str})
    df.columns = [c.strip() for c in df.columns]
    # Fecha desde columna o desde nombre del archivo
    if "Fecha de Vencimiento" in df.columns:
        df["Fecha de Vencimiento"] = pd.to_datetime(df["Fecha de Vencimiento"], errors="coerce")
    fecha_nombre = nombre_archivo.replace(".xlsx","").replace(".xls","")
    if df.get("Fecha de Vencimiento", pd.Series()).isna().all() if "Fecha de Vencimiento" in df.columns else True:
        df["Fecha de Vencimiento"] = pd.to_datetime(fecha_nombre, format="%Y%m%d", errors="coerce")
    df["_archivo"] = fecha_nombre
    return df
...
with st.sidebar:
    ...
    # Descargar produccion_real.json
    st.markdown('<div class="upload-title">💾 Guardar avances del día</div>', unsafe_allow_html=True)
    st.caption("Descarga el archivo de avances para no perder lo ingresado. Guárdalo y súbelo mañana.")
    json_bytes = json.dumps(st.session_state.produccion_real, ensure_ascii=False, indent=2).encode("utf-8")
    st.download_button(
        "⬇️ Descargar produccion_real.json",
        data=json_bytes,
        file_name="produccion_real.json",
        mime="application/json",
        use_container_width=True
    )

    st.markdown("---")
    st.caption("🏭 **Control de Producción v4.0 Cloud**")

# ── HEADER ────────────────────────────────────────────────────────────────────
st.markdown("""
<div class="main-header">
  <h1>🏭 Control de Producción</h1>
  <p>Seguimiento en tiempo real · Versión Cloud · Multi-archivo por día</p>
</div>
""", unsafe_allow_html=True)
...
if falta_prod or falta_prog:
    ...
    with col_a:
        estado = "estado-no" if falta_prod else "estado-ok"
        texto  = "❌ Sin cargar" if falta_prod else f"✅ {len(df_productos)} productos"
        st.markdown(f'<div class="upload-box"><div class="upload-title">Maestro de Productos</div>'
                    f'<span class="{estado}">{texto}</span></div>', unsafe_allow_html=True)
    with col_b:
        estado = "estado-no" if falta_prog else "estado-ok"
        n_dias = df_programacion["_archivo"].nunique() if not falta_prog else 0
        texto  = "❌ Sin cargar" if falta_prog else f"✅ {n_dias} día(s) cargado(s)"
        st.markdown(f'<div class="upload-box"><div class="upload-title">Programación Diaria</div>'
                    f'<span class="{estado}">{texto}</span></div>', unsafe_allow_html=True)

    st.info("👈 Carga los archivos desde la barra lateral para comenzar.")
    st.stop()
...
df_base["CANT_PLAN"]  = df_base["CANT_PLAN"].fillna(0).round(2)
...
dias_cargados = int(df_programacion["_archivo"].nunique())
...
if total_batch > 0:
    ...
    <div style="background:#e0f7fa;border:1px solid #b2ebf2;border-radius:10px;
      ...
      <div style="background:#b2ebf2;border-radius:6px;height:14px;overflow:hidden;">
        <div style="width:{min(pct_batch,100):.1f}%;height:100%;
                    background:{color};border-radius:6px;"></div>
      </div>
      <div style="display:flex;justify-content:space-between;margin-top:5px;
                  font-size:0.78rem;color:#0e7490;">
...
with tab1:
    ...
    def color_estado(val):
        return {
            "Completado":       "background-color:#0d2a1a;color:#4ecf8c;font-weight:600",
            ...
            "Sin programación": "background-color:#12192a;color:#5a7090",
        }.get(val, "")
    ...
    styled = df_tabla.style \
        ...
        .map(color_avance, subset=["Avance %"]) \
        .format({"Cant. Plan":"{:,.2f}","Cant. Real":"{:,.2f}",
                 "Cant. Pend.":"{:,.2f}","Avance %":"{:.1f}%"}) \
        .hide(axis="index")
...
with tab2:
    ...
    if df_con_prog.empty:
        st.info(f"ℹ️ No hay productos programados para el {fecha_sel.strftime('%d/%m/%Y')} con los filtros actuales.")
    else:
        subtitulo = f"**{len(df_con_prog)}** productos programados para el **{fecha_sel.strftime('%d/%m/%Y')}**"
        if linea_sel != "— Todas las líneas —":
            subtitulo += f" — Línea: **{linea_sel}**"
        st.markdown(subtitulo)
        st.caption("⚠️ Recuerda descargar el `produccion_real.json` al final del día desde la barra lateral para no perder los avances.")
        st.markdown("")
        ...
        for pos, (_, row) in enumerate(df_con_prog.iterrows()):
            ...
            with st.expander(titulo, expanded=(br > 0 and pend_b > 0)):
                ...
                mc1.metric("✅ BATCH Producido", br,
                           delta=f"-{pend_b} pendiente" if pend_b > 0 else "✓ Completo")
                mc2.metric("📦 Cant. Producida", f"{cr:,.2f}",
                           delta=f"-{pend_c:,.2f} pend." if pend_c > 0 else "✓ Completo")
                ...
                with ci1:
                    nuevo_batch = st.number_input(
                        "BATCH producidos", min_value=0, max_value=bp*3,
                        value=br, step=1, key=f"batch_{ukey}"
                    )
                with ci2:
                    nueva_cant = st.number_input(
                        "Cantidad producida", min_value=0.0, max_value=float(cp*3),
                        value=float(cr), step=0.5, format="%.2f", key=f"cant_{ukey}"
                    )
                with ci3:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("💾 Guardar", key=f"save_{ukey}", use_container_width=True):
                        st.session_state.produccion_real[k] = {
                            "batch_real": nuevo_batch, "cant_real": nueva_cant,
                            "timestamp": datetime.now().isoformat(),
                            "codigo": row["CODIGO"], "producto": row["PRODUCTO"], "fecha": fecha_str,
                        }
                        produccion_real = st.session_state.produccion_real
                        st.success(f"✅ Guardado — Pend: {max(bp-nuevo_batch,0)} BATCH · {max(cp-nueva_cant,0):,.2f} unid.")
        ...
        if st.button("💾 GUARDAR TODO DE UNA VEZ", type="primary", use_container_width=True):
            ...
            for pos, (_, row) in enumerate(df_con_prog.iterrows()):
                ...
                if bk in st.session_state:
                    st.session_state.produccion_real[k] = {
                        "batch_real": st.session_state[bk], "cant_real": st.session_state[ck],
                        "timestamp": datetime.now().isoformat(),
                        "codigo": row["CODIGO"], "producto": row["PRODUCTO"], "fecha": fecha_str,
                    }
                    cambios += 1
            produccion_real = st.session_state.produccion_real
            st.success(f"✅ {cambios} registros guardados — descarga el JSON desde la barra lateral 👈")
...
with tab3:
    ...
    df_todos["CANT_PLAN"]  = df_todos["CANT_PLAN"].fillna(0).round(2)
    ...
    for linea in sorted(df_todos["LINEA"].dropna().unique()):
        ...
        br_l = sum(produccion_real.get(key_reg(fecha_str, r["CODIGO"]), {}).get("batch_real", 0)
                   for _, r in sub.iterrows())
        cr_l = sum(produccion_real.get(key_reg(fecha_str, r["CODIGO"]), {}).get("cant_real", 0.0)
                   for _, r in sub.iterrows())
        resumen_linea.append({
            "Línea":            linea,
            "Productos":        len(sub),
            "Con Prog.":        int((sub["BATCH_PLAN"] > 0).sum()),
            "BATCH Plan":       bp_l,
            "BATCH Real":       br_l,
            "BATCH Pend.":      max(bp_l - br_l, 0),
            "Avance % (BATCH)": round(br_l / bp_l * 100, 1) if bp_l > 0 else 0.0,
            "Cant. Plan":       round(cp_l, 2),
            "Cant. Real":       round(cr_l, 2),
            "Variación Cant.":  round(cr_l - cp_l, 2),
        })

    df_lineas = pd.DataFrame(resumen_linea)

    st.markdown('<div class="section-title">📊 Avance por BATCH — Métrica principal</div>',
                unsafe_allow_html=True)
    ...
    styled_b = df_lineas[cols_b].style \
        .map(bg_avance, subset=["Avance % (BATCH)"]) \
        .format({"Avance % (BATCH)": "{:.1f}%"}) \
        .hide(axis="index")
    st.dataframe(styled_b, use_container_width=True, height=460)

    df_g = df_lineas[df_lineas["BATCH Plan"] > 0].set_index("Línea")[["BATCH Plan","BATCH Real"]]
    if not df_g.empty:
        st.markdown('<div class="section-title">BATCH Plan vs Real por Línea</div>',
                    unsafe_allow_html=True)
        st.bar_chart(df_g, use_container_width=True, height=300)

    st.markdown('<div class="section-title">🔬 Análisis de Cantidades — Referencial</div>',
                unsafe_allow_html=True)
    st.caption("La variación muestra cuánto se produjo de más (+) o de menos (–) respecto a lo planificado.")
    ...
    styled_c = df_lineas[cols_c].style \
        .map(color_var, subset=["Variación Cant."]) \
        .format({"Cant. Plan":"{:,.2f}","Cant. Real":"{:,.2f}","Variación Cant.":"{:+,.2f}"}) \
        .hide(axis="index")
    st.dataframe(styled_c, use_container_width=True, height=460)
...
with tab4:
    st.markdown('<div class="section-title">🕐 Pendientes Históricos por Producto — Método FIFO</div>',
                unsafe_allow_html=True)
    st.markdown("Selecciona un producto y ve todos sus BATCH pendientes ordenados de **más antiguo a más reciente**. "
                "Declara siempre desde la fecha más antigua.")
    ...
    if hist_pend.empty:
        ...
        prods_pend = hist_pend.copy()
        if linea_pend != "— Todas —":
            prods_pend = prods_pend[prods_pend["LINEA"] == linea_pend]

        lista_prods = sorted(
            prods_pend[["CODIGO","PRODUCTO"]].drop_duplicates()
        ...
        if not detalle_prod.empty:
            ...
            total_pend_prod  = int(detalle_prod["BATCH_PEND"].sum())
            ...
            <div style="background:#f0f9ff;border-radius:10px;padding:0.8rem 1.2rem;
              ...
              <span style="color:#b91c1c;font-size:0.95rem;font-weight:700;">{total_pend_prod}</span>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("**Declara la producción empezando siempre por la fecha más antigua ↓ (FIFO)**")
            ...
            for fpos, (_, frow) in enumerate(detalle_prod.iterrows()):
                ...
                badge = ('<span style="background:#b91c1c;color:#fff;font-size:0.72rem;'
                         'padding:2px 10px;border-radius:12px;font-weight:700;">⬆ DECLARAR PRIMERO</span>'
                         if fpos == 0 else
                         f'<span style="background:#e0f7fa;color:#0e7490;font-size:0.72rem;'
                         f'padding:2px 10px;border-radius:12px;">#{fpos+1} en cola</span>')
                ...
                with st.expander(f"📅 {f_disp}  —  {pend_b_f} BATCH pendientes", expanded=(fpos == 0)):
                    ...
                    <div style="background:#f0f9ff;border-radius:8px;padding:0.5rem 1rem;
                      ...
                      &nbsp;&nbsp;{badge}&nbsp;&nbsp;
                      <span style="color:#555;">BATCH Plan: </span>
                      <span style="color:#1a1a2e;font-weight:600;">{bp_f}</span>
                      &nbsp;&nbsp;
                      <span style="color:#555;">Ya producido: </span>
                      <span style="color:#0e7490;font-weight:600;">{br_f}</span>
                      &nbsp;&nbsp;
                      <span style="color:#555;">Pendiente: </span>
                      <span style="color:#b91c1c;font-weight:700;">{pend_b_f}</span>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with fi1:
                        nb_fifo = st.number_input(
                            "BATCH a declarar", min_value=0, max_value=bp_f*2,
                            value=br_f, step=1, key=f"nbatch_{ukey_f}"
                        )
                    with fi2:
                        nc_fifo = st.number_input(
                            "Cantidad a declarar", min_value=0.0, max_value=float(cp_f*2),
                            value=float(cr_f), step=0.5, format="%.2f", key=f"ncant_{ukey_f}"
                        )
                    with fi3:
                        st.markdown("<br>", unsafe_allow_html=True)
                        if st.button("💾 Guardar", key=f"nsave_{ukey_f}", use_container_width=True):
                            st.session_state.produccion_real[k_f] = {
                                "batch_real": nb_fifo, "cant_real": nc_fifo,
                                "timestamp": datetime.now().isoformat(),
                                "codigo": codigo_sel, "producto": nombre_prod, "fecha": f_str,
                            }
                            produccion_real = st.session_state.produccion_real
                            st.success(f"✅ Guardado — Pendiente restante: {max(bp_f-nb_fifo,0)} BATCH")
                            import time; time.sleep(0.3)
                            st.rerun()

            st.markdown("---")
            st.markdown("**Resumen de todas las fechas para este producto:**")
            df_fr = detalle_prod[["fecha_str","BATCH_PLAN","BATCH_REAL","BATCH_PEND",
                                  "CANT_PLAN","CANT_REAL","CANT_PEND"]].copy()
            df_fr.columns = ["Fecha","BATCH Plan","BATCH Real","BATCH Pend.",
...
st.markdown(
    ...
    f'🏭 Control de Producción v4.0 Cloud · {n_dias} día(s) cargado(s) · '
